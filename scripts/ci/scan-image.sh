...
(cd "$work/layers" && grep -rIlE "$key_patterns" . 2>/dev/null | sed 's|^|key-shaped: |') |
  filter_allowed >>"$findings" || true

(cd "$work/layers" && grep -rIlE '\b[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}\b' --exclude-dir=node_modules . 2>/dev/null |
  sed 's|^|cpf-shaped: |') | filter_allowed >>"$findings" || true

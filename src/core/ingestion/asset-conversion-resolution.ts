...
export interface AssetConversionLegPlan {
  ...
  /** SPEC-007 BR-007-05b: a conversion carries cost, never cash — always zero. */
...
export interface ResolvedAssetConversion {
  ...
  /** The cost the incoming legs carry: exactly Σ removed (SPEC-007 BR-007-05b). */
 ...
 * restated balance: same-day price-less `Transferência` credits (#121).
 * Anywhere else two statements of one code are two readings of one balance,
 ...
 * actually moved them. Read as *what remains after the conversion*, the
 * statement leaves nothing removed and the whole group refuses
 * `insufficient_quantity` — so the statement is set aside, and the source
 * contributes what the rest of the group says it did (its own outgoing
 * evidence, or its whole position where the evidence is target-only).
 *
 * Only an **exactly** unchanged balance, only an `Atualização`, only on a code
 * a definition sources **and** opts in with `sourceBalanceRestatements`. A
 * statement that differs from the replay by any amount is still read as what
 * remains; a target's unchanged statement still
...
export function resolveAssetConversion(
  ...
  // SPEC-007 BR-007-05b: a conversion carries cost and never cash, so the
  // incoming legs receive exactly the cost the outgoing legs removed.
  ...
  const outLegs = sourcePlans.map(({ assetCode, quantity, removedCost }) => {

...
import { isConsentPurpose, type CONSENT_PURPOSES } from '@/core/privacy/ports';
...
export async function requestAccountDeletionAction(): Promise<void> {
  const userId = await requireUserId();
  const deletionWindowDays = (await resolveConfig('retention.deletion_window_days', { db })).value;
